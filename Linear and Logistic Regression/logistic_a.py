...
def logistic_regression(X,Y,strategy,lr,max_iter):
    W = np.zeros((X.shape[1],Y.shape[1]))
    for i in range(max_iter):
        rate = float(parameter[1][0])
        Z = softmax(np.dot(X,W))
        gradient = np.dot(X.T,(Z-Y))/Y.shape[0]
        if(i%1000==0):
            print(-np.sum(np.multiply(Y,np.log(softmax(np.dot(X,W)))))/2*Y.shape[0])
        if(strategy==1):
            W -= lr*gradient
        elif(strategy==2):
...
X = preprocessX(data)
Y = preprocessY(data)
W = logistic_regression(X,Y,int(parameter[0][0]),float(parameter[1][-1]),int(parameter[2][0]))
